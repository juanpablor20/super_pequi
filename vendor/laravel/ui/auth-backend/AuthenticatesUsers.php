...
trait AuthenticatesUsers
{
    ...
    public function login(Request $request)
    {
        ...
        if ($loginResult === 'inactive') {
            return redirect()->back()->withErrors(['users' => 'El usuario está inactivo']);
        }
    ...
    protected function attemptLogin(Request $request)
    {
        ...
        if ($user->states === 'inactive') {
            Log::warning('El usuario con number_identification: ' . $login->users . ' está inactivo.');
            return 'inactive';
        }
    ...
    protected function authenticated(Request $request, $user)
    {
        ...
        if ($login) {
            ...
            if ($usuario && $usuario->states === 'inactive') {
                // Manejar error de usuario inactivo
                return $request->wantsJson()
                    ? new JsonResponse(['error' => 'El usuario está inactivo'], 403)
                    : redirect()->back()->withErrors(['users' => 'El usuario está inactivo']);
            }
